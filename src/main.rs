...
fn main() {
    let matches = App::new("airscan")
        ...
        .arg(
            Arg::with_name("udp")
                .short("u")
                .long("udp")
                .help("Runs the test with UDP instead of TCP"),
        )
        .get_matches();
    ...
    let with_udp = matches.occurrences_of("udp") == 1;

    let start = Instant::now();

    let udpsocket = UdpSocket::bind("0.0.0.0:14141").unwrap();
    ...
    for ip in ips {
        ...
        for port in &ports {
            if !with_udp {
                ...
                let open = match udpsocket.connect(&format!("{}:{}", ip, port)) {
                    Ok(_) => true,
                    Err(_e) => false,
                };
                if open {
        ...
        println!(
            "\nTotal : {} ports scanned : {} open, {} closed.",
            open_amount + closed,
            open_amount,
            closed
        );
